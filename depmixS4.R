...
#fish <- rename(fish, Time = dt)
fish <- read.csv(".\\data\\filtered_data_1_grayling_46909_HPE_RMSE_Vel.csv")

fish$Time <- as.POSIXct(fish$Time)

# Set time resolutions and construct movement model
time_res <- c('5 secs', '10 secs', '15 secs', '30 secs', '60 secs', '2 mins', '5 mins')
fishreg <- crwMLE(data = na.omit(Rfish), coord = c('UTM.x', 'UTM.y'), Time.name = c("Time") , time.scale = 'secs')

# Read shapefile for map
...
#Start for-loop with diff timeres
for (res in time_res){
  # Construct movement model and regularize data
  fishregmod <- crwPredict(object.crwFit = fishreg, 
                           predTime = seq.POSIXt(from = min(na.omit(Rfish$Time)), to = max(na.omit(Rfish$Time)), by = res), 
                           return.type = 'minimal')
  ...
  HMMdat2 <- moveHMM::prepData(HMMdat, type='UTM') # 35
  ...
  HMMmod <- depmix(list(step ~ 1,SI ~ 1), data = HMMdat2, 
                   family = list(Gamma(), gaussian()), respstart = c(1, 2, a, b, 1, 1), # ==> Why 6 values? 2 for steps, 2 for SI, 2 for????
